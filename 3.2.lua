--[[
	V2 Written by Tunicus (20/10/2017)
	V3 Modified by glitchifyed (13/11/2021)
	Latest revision: V3.2
	V2 Model link: https://www.roblox.com/library/1122124371
	V3+ Model link: https://www.roblox.com/library/7980966840
	V3 Developer Forum post: https://devforum.roblox.com/t/1547553
	...
	V3.2 (23/11/2024)
		- Refactored some of the code, added a little bit of type checking
		- The damping now no longer speeds up at an FPS larger than 240hz
		- A hydraulic toggle has been added that allows you to completely disable hydraulics altogether
		- A platform toggle has also been added that when disabled will completely ignore platforms altogether
		- The platformSize ReplicatedStorage NumberValue has been removed and has been replaced with just a normal variable named PLATFORM_SIZE. It still corresponds to the Y size of your platform objects
	V3.1.4 (15/09/2024)
		- Fixed the broken grid texture
		- Removed some redundant variables
	V3.1.3 (29/05/2024) Changelog
		- Removed a few redundancies in the code
		- Fixed up the versioning a bit
	
	V3.1.2 (24/7/2022) Changelog
		- Hydraulic item legs have been completely rewritten and should now work at any y-position
	...
	Usage instructions
		...
		I'd imagine you actually want to place the model itself, and since you're a competent game developer that uses FilteringEnabled, you'll have to do this server sided. Here's some untested sample code to give you
			...
			LOCAL SCRIPT
					...
					local itemPlacingFolder = workspace.TemporaryPlacements
...
local HYDRAULICS_ENABLED = false -- whether hydraulics can be utilised or not
local PLATFORMS_ENABLED = false -- whether platforms can be utilised or not
...
local PLATFORM_SIZE = 1 -- the y size of platform parts (used to offset the placement correctly on platforms)
...
--// GRID SETTINGS
local GRID_TEXTURE = "rbxassetid://83794238717677" -- texture of the grid space, set to nil if you don't want a visible grid
...
local function setObjectY(obj)
	if (not HYDRAULICS_ENABLED) then
		if (not PLATFORMS_ENABLED) then
			obj.model:SetAttribute("y", 0)
			
			return
		end
		
		obj.model:SetAttribute("y", obj.platformY or 0)
...
local function inputCapture() -- converts user inputs to 3d position, built in mobile compatibility
	...
	rayParams.FilterDescendantsInstances = PLATFORMS_ENABLED and {currentPlane.base, table.unpack(currentPlatforms)} or {currentPlane.base}
	...
	if not unrest or currentPlatform or newHit or lastPlatforms ~= platformNumber then
		...
		for _, obj in pairs(currentObjects) do
			...
			if currentPlatform and currentHit and no_object_on_platforms then
				...
				if pointsInsidePlatform then
					platformY = (currentHit.Position.Y + PLATFORM_SIZE / 2 - currentBase.Position.Y - currentBase.Size.Y / 2) / currentPlane.grid
				end
			end
...
local function render(delta)
	...
	if (INTERPOLATION) then
		...
		if (typeof(delta) == "number") then
			current_delta_time = math.min(delta, 1 / 30)
		else
			delta = current_delta_time
		end
...
local function place()
	if (currentPlane and (obstacleCollision()) and (HOLD_TO_PLACE or (tick() - lastPlacement) >= PLACEMENT_COOLDOWN) and not currentPlane.loading) then
		...
		local modelCFrames = {}
		local modelYPositions = (HYDRAULICS_ENABLED or PLATFORMS_ENABLED) and {heights = HYDRAULICS_ENABLED and {} or nil, platforms = PLATFORMS_ENABLED and {} or nil} or nil
		...
		print(modelCFrames, modelYPositions)		
	end
end
...
local function enablePlacement(plane, models: {Model} | Model, previewObject: Instance, prealigned: boolean)
	...
	obstacleAdded = currentPlane.obstacles.ChildAdded:Connect(function(obj)
		if not obj.PrimaryPart then
			local finish_check = tick() + 0.5
			
			repeat
				task.wait()
				
				if tick() >= finish_check then
					return
				end
			until obj.PrimaryPart
		end
		
		if obj:IsA("Model") and obj.PrimaryPart then
			createSelectionBox(obj, true)
			
			if PLATFORMS_ENABLED and obj:FindFirstChild("platform") and obj.platform:IsA("BasePart") then
				table.insert(currentPlatforms, obj.platform)
...
local function disablePlacement(plane)
	if (currentPlane) then
		...
		if (obstacleAdded) then
			obstacleAdded:Disconnect()
			obstacleAdded = nil
		end
		
		if (obstacleRemoved) then
...
module.new = function(base: BasePart, obstacles: Instance, grid: number)
	local plane: {
		enable: (plane: any, models: {Model} | Model, previewObject: Instance, prealigned: boolean) -> (),
		disable: (plane: any) -> (),
		
		rotate: (plane: any) -> (),
		place: (plane: any) -> (),
		
		setLoading: (plane: any, isLoading: boolean) -> (),
	} = {}
	plane.base = base
